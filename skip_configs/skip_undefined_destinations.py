...
class SkipDestinationUndefinedConfig(Config):
    ...
    def resolve_helper(self, resolve_params):
        ...
        if not enabled and resolve_params.dst_dirpath == "":
            toc.error(f"Failed to back up the file '{resolve_params.src_filepath}'. Could not resolve destination. Notify Ryan or whoever is maintaining the backup scripts. Press enter to continue.")
            input()
            return True
